...
int main() {
  ...
  while (1) { // Loop infinitely
    ...
    if (left_IR == 1 && right_IR == 0) { // If left sensor detects line and right sensor does not
      // Turn left
      servo_speed(leftServo, -50); // Set left motor speed to maximum in reverse direction
      servo_speed(rightServo, -50); // Set right motor speed to maximum in forward direction
    } else if (left_IR == 0 && right_IR == 1) { // If right sensor detects line and left sensor does not
      // Turn right
      servo_speed(leftServo, 50); // Set left motor speed to maximum in forward direction
      servo_set(rightServo, 50); // Set right motor speed to maximum in reverse direction
